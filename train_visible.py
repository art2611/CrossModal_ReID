...
import time
from data_loader import RegDBData, RegDB_Visible_Data, GenIdx, process_test_regdb, TestData
from torchvision import transforms
import matplotlib.pyplot as plt
import numpy as np
from utils import VisibleIdentitySampler, AverageMeter, adjust_learning_rate
...
def multi_process() :
    ...
    Timer1 = time.time()
    ######################################### TRAIN SET
    trainset = RegDB_Visible_Data(data_path, trial = 1, transform=transform_train)
    ...
    color_pos, _ = GenIdx(trainset.train_color_label, trainset.train_color_label)
    ...
    n_query = len(gall_visible_label)
    n_gall = len(query_visible_label)
    ...
    print('  ------------------------------')
    print(f'  query    | {len(np.unique(query_visible_label)):5d} | {n_query:8d}')
    print(f'  gallery  | {len(np.unique(gall_visible_label)):5d} | {n_gall:8d}')
    print('  ------------------------------')
    ...
    def test_visible(epoch):
        # Get all normalized distance
        gall_feat_pool, gall_feat_fc = extract_gall_feat(gall_loader, n_gall, net=net, visible_train=True)
        query_feat_pool, query_feat_fc = extract_query_feat(query_loader, n_query, net=net, visible_train=True)
        ...
        cmc, mAP, mINP = eval_regdb(-distmat_pool, query_visible_label, gall_visible_label)
        cmc_att, mAP_att, mINP_att = eval_regdb(-distmat_fc, query_visible_label, gall_visible_label)
    ...
    for epoch in range(81):
        ...
        sampler = VisibleIdentitySampler(trainset.train_color_label, \
                                  color_pos, num_of_same_id_in_batch, trainV_batch_num_identities,
                                  epoch)
        ...
        if epoch > 0 and epoch % 2 == 0:
            ...
            # testing
            cmc, mAP, mINP, cmc_att, mAP_att, mINP_att = test_visible(epoch)
            # save model

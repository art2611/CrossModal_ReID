...
class RegDB_Visible_Data(data.Dataset):
    def __init__(self, data_dir, trial, transform=None, colorIndex=None):
        # Load training visible images (path) and labels
        data_dir = '../Datasets/RegDB/'
        train_color_list = data_dir + 'idx/train_visible_{}'.format(trial) + '.txt'
        # Load color images
        color_img_file, train_color_label = load_data(train_color_list)

        # Get visible images with good shape in a list
        train_color_image = []
        for i in range(len(color_img_file)):
            img = Image.open(data_dir + color_img_file[i])
            img = img.resize((144, 288), Image.ANTIALIAS)
    ...
    def __getitem__(self, index):
        # Dataset[i] return the visible image and the corresponding label
        img1, target1 = self.train_color_image[self.cIndex[index]], self.train_color_label[self.cIndex[index]]
        img1 = self.transform(img1)
        return img1, target1
